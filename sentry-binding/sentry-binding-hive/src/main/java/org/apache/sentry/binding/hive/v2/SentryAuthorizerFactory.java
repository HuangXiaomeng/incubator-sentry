...
public class SentryAuthorizerFactory implements HiveAuthorizerFactory {
  ...
  public HiveAuthorizer createHiveAuthorizer(HiveMetastoreClientFactory metastoreClientFactory,
    ...
    SentryAccessController accessController =
        getAccessController(conf, authzConf, hiveAuthzBinding, authenticator, sessionContext);
    SentryAuthorizationValidator authzValidator =
        getAuthzValidator(conf, authzConf, hiveAuthzBinding, authenticator);
  ...
  public HiveAuthorizer createHiveAuthorizer(HiveMetastoreClientFactory metastoreClientFactory,
    ...
    SentryAccessController accessController =
        getAccessController(conf, authzConf, hiveAuthzBinding, authenticator, ctx);
    SentryAuthorizationValidator authzValidator =
        getAuthzValidator(conf, authzConf, hiveAuthzBinding, authenticator);
    ...
    try {
      Constructor<? extends SentryAccessController> constructor =
          clazz.getConstructor(HiveAuthzConf.class, HiveAuthzBinding.class,
              HiveAuthenticationProvider.class, HiveAuthzSessionContext.class);
      accessController = (SentryAccessController)
          constructor.newInstance(authzConf, hiveAuthzBinding, authenticator, ctx);
    } catch (Exception e) {
      throw new HiveAuthzPluginException(e);
    }
